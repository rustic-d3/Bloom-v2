...
import { chosenDatesCalculation, calculateDaysCount, removeItemFromArray, getOpacityColorStyle } from "../../utils/generalUtils";
...
export const CalendarHeader = props => {
  ...
  const selectedDaysStyle = getOpacityColorStyle(selectedColor, 60); // language not included in funciton call because of the css ltr property of hebrew case

  let chosenDates = chosenDatesCalculation(selectedDays, hoveredDay, format, pickMethod);
  chosenDates = addDatesCount(pickMethod, selectedDays, chosenDates, language, hoveredDay);

  const handleClearClick = () => {
    if (pickMethod === "ranges") {
      let clearedChosenDatesList = removeItemFromArray(chosenDatesList, chosenDates);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    lang: language
  }, /*#__PURE__*/React.createElement(DatesDisplay, {
...
function addDatesCount(pickMethod, selectedDays, chosenDates, language, hoveredDay) {
  if (pickMethod !== "date") {
    if (selectedDays.length === 2) {
      chosenDates += calculateDaysCount(selectedDays[0], selectedDays[1], language);
...
function areEqual(prevProps, nextProps) {
  return prevProps.selectedDays === nextProps.selectedProps && prevProps.selectedColor === nextProps.selectedColor;
}

export default /*#__PURE__*/React.memo(CalendarHeader, areEqual);