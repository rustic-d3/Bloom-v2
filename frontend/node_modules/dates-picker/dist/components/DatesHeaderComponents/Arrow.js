...
export const Arrow = props => {
  ...
  const canIncrease = () => {
    const isNearMonthNotBlocks = nearViewedMonths.right.year ? new Date(viewedYear, viewedMonth + 2, 0) <= new Date(nearViewedMonths.right.year, nearViewedMonths.right.month, 0) : true;
    const isSmallerThanEndDate = new Date(viewedYear, viewedMonth + 1, 0) < endDate;
    return isNearMonthNotBlocks && isSmallerThanEndDate;
  };

  const canDecrease = () => {
    const isNearMonthNotBlocks = nearViewedMonths.left.year ? new Date(viewedYear, viewedMonth - 2, 0) >= new Date(nearViewedMonths.left.year, nearViewedMonths.left.month, 0) : true;
